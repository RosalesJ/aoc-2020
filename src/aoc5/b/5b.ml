...
let binary_boarding seat_codings =
  ...
  let occupied =
    let seat_ids = List.map ~f:(decode_seat plane) seat_codings in
    List.fold seat_ids ~f:Set.add ~init:(Set.empty (module Int))
  in

  List.find (List.concat seats) ~f:(is_my_seat occupied)
